...
def epochs_spkcount(neurons: Union[core.Neurons, pd.DataFrame], epochs: Union[core.Epoch, pd.DataFrame], bin_size=0.01, slideby=None, export_time_bins:bool=False, included_neuron_ids=None, debug_print:bool=False, use_single_time_bin_per_epoch: bool=False):
    ...
    for i, epoch in enumerate(epoch_df.itertuples()):
        ...
        if epoch.stop <= epoch.start:
            if debug_print:
                print(f"Skipping epoch {i} with non-positive duration.")
        ...
        if use_single_time_bin_per_epoch:
            ...
            bin_starts = np.arange(epoch.start, (epoch.stop - bin_size + 1e-10), slideby)
            if len(bin_starts) == 0:
                # Epoch shorter than bin_size
                bin_starts = np.array([epoch.start])
                bin_ends = np.array([epoch.stop])
        ...
        if export_time_bins:
            ...
            # Create BinningInfo for edges and centers
            edge_info = BinningInfo(
                variable_extents=(bins[0], bins[-1]),
                step=np.diff(bins).mean(),
                num_bins=len(bins)
            )
            center_info = BinningInfo(
                variable_extents=(bin_centers[0], bin_centers[-1]) if len(bin_centers) > 1 else (bin_centers[0], bin_centers[0]),
                step=np.diff(bin_centers).mean() if len(bin_centers) > 1 else bins[-1] - bins[0],
                num_bins=len(bin_centers)
            )

            # Create BinningContainer
        ...
        # end if export_time_bins

        spkcount.append(spkcount_)
