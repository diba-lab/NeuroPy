...
Updated by Nat Kinsky 20244-11-07 01:31:00.
'''
...
def getProminence(var,step,lats=None,lons=None,min_depth=None,
    ...
    # Check version of matplotlib if greater than 3.5 then you have to split contours manually
    ### NRK todo: check which version this switches from an iterable of Paths as an output of `contour` to just one Path
    split_contour_flag = False
    if Version(matplotlib.__version__) > Version("3.6"):
        split_contour_flag = True
    ...
    if area_func==contourGeoArea:
        assert False, 'contourGeoArea debugging'
        # from mpl_toolkits.basemap import Basemap
        # lat1=np.min(lats)
        # lat2=np.max(lats)
        # lon1=np.min(lons)
    ...
    for ii,levii in enumerate(levels[::-1]):
        ...
        if include_edge:
            ...
            if not split_contour_flag:
                csii_list = csii.collections[0].get_paths()
            else:
                csii_list = split_path(csii.get_paths()[0])
            ax.cla()
        else:
            csii=contours[ii]
        ...
        for jj, contjj in enumerate(csii_list):
...
def split_path_by_index(path, index):
    """Splits a Matplotlib Path object into two at the given index."""
    ...
    path1_verts = deepcopy(vertices[:index])
    path1_codes = deepcopy(codes[:index])
    if path1_codes[-1] != Path.CLOSEPOLY:
        path1_codes[-1] = Path.STOP  # End the first path properly

    # Create the second path
    path2_verts = deepcopy(vertices[index:])
    path2_codes = deepcopy(codes[index:])
    if path2_codes[0] != Path.MOVETO:
        path2_codes[0] = Path.MOVETO  # Start the second path with a MOVETO

    return Path(path1_verts, path1_codes), Path(path2_verts, path2_codes)


def split_path(path, splitby=Path.CLOSEPOLY):

    if len(path) > 0:
        npts = len(path.codes)
        split_inds = np.where(path.codes == splitby)[0] + 1
        npoly = len(split_inds)
        split_path_list = []
        last_poly_ind = 0
        for poly in range(npoly):
            ...
            if poly < (npoly - 1):
                ...
                split_temp = [path]
            split_path_list.append(split_temp[0])
    elif len(path) == 0:
        split_path_list = []
...
if __name__=='__main__':
    ...
    peaks,idmap,promap,parentmap=getProminence(slab,step,lats=yy,lons=xx,min_area=None,
            include_edge=True, area_func=contourArea)
