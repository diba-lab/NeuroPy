...
from ephyviewer.base import (
    BaseMultiChannelViewer,
    Base_MultiChannel_ParamController,
    MyViewBox,
)
from ephyviewer.datasource import InMemoryAnalogSignalSource

from ephyviewer.tools import create_plot_grid, get_dict_from_group_param
...
class SpectrogramViewer(BaseMultiChannelViewer):
    ...
    def __init__(self, freq_lim=(0, 60), **kargs):
    ...
    def on_data_ready(self, chan, t, t_start, t_stop, t1, t2, Sxx):
        ...
        f_start, f_stop = self.freq_lim
        freqs = np.linspace(0, self.source.sample_rate / 2.0, Sxx.shape[0])
        indices = (freqs >= f_start) & (freqs <= f_stop)
        Sxx = gaussian_filter(Sxx[indices, :], sigma=0.7)
