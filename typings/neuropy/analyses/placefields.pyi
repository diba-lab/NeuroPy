...
class PlacefieldComputationParameters(SimplePrintable, DiffableObject, SubsettableDictRepresentable, metaclass=OrderedMeta):
    ...
    def str_for_attributes_list_display(self, param_sep_char=..., key_val_sep_char=..., subset_includelist: Optional[list] = ..., subset_excludelist: Optional[list] = ..., override_float_precision: Optional[int] = ..., override_array_items_threshold: Optional[int] = ...):
...
class PfnDMixin(SimplePrintable):
    ...
    def plot_raw(self, subplots=..., fignum=..., alpha=..., label_cells=..., ax=..., clus_use=...):
...
class PfND(HDFMixin, ContinuousPeakLocationRepresentingMixin, PeakLocationRepresentingMixin, NeuronUnitSlicableObjectProtocol, BinnedPositionsMixin, PfnConfigMixin, PfnDMixin, PfnDPlottingMixin):
    ...
    @classmethod
    def from_config_values(cls, spikes_df: pd.DataFrame, position: Position, epochs: Epoch = ..., frate_thresh=..., speed_thresh=..., grid_bin=..., grid_bin_bounds=..., smooth=..., setup_on_init: bool = ..., compute_on_init: bool = ...):
        """ initialize from the explicitly listed arguments instead of a specified config. """
        ...
    
    def setup(self, position: Position, spikes_df, epochs: Epoch, debug_print=...):
    ...
    def conform_to_position_bins(self, target_pf, force_recompute=...):
    ...
    @staticmethod
    def build_position_df_discretized_binned_positions(active_pos_df, active_computation_config, xbin_values=..., ybin_values=..., debug_print=...):
...
def perform_compute_placefields(active_session_spikes_df, active_pos, computation_config: PlacefieldComputationParameters, active_epoch_placefields1D=..., active_epoch_placefields2D=..., included_epochs=..., should_force_recompute_placefields=..., progress_logger=...):
...
def compute_placefields_masked_by_epochs(sess, active_config, included_epochs=..., should_display_2D_plots=...):
    """ Wrapps perform_compute_placefields to make the call simpler """
    ...

def compute_placefields_as_needed(active_session, computation_config: PlacefieldComputationParameters = ..., general_config=..., active_placefields1D=..., active_placefields2D=..., included_epochs=..., should_force_recompute_placefields=..., should_display_2D_plots=...):
    ...
