...
def add_inner_title(ax, title, loc, strokewidth=..., stroke_foreground=..., text_foreground=..., **kwargs):
...
def scale_title_label(ax, curr_title_obj, curr_im, debug_print=...):
...
def add_value_labels(ax, spacing=..., labels=...):
...
def plot_position_curves_figure(position_obj, include_velocity=..., include_accel=..., figsize=...):
    """ Renders a figure with a position curve and optionally its higher-order derivatives """
    ...

def draw_epoch_regions(epoch_obj, curr_ax, facecolor=..., edgecolors=..., alpha=..., labels_kwargs=..., defer_render=..., debug_print=..., **kwargs):
...
class MatplotlibFigureExtractors:
    ...
    @classmethod
    def extract_titles(cls, fig: Optional[Figure] = ...):
        """ 
...
def interactive_select_grid_bin_bounds_1D(curr_active_pipeline, epoch_name=...):
...
def interactive_select_grid_bin_bounds_2D(curr_active_pipeline, epoch_name=..., should_block_for_input: bool = ..., should_apply_updates_to_pipeline=...):
...
def perform_update_title_subtitle(fig, ax, title_string: Optional[str], subtitle_string: Optional[str], active_context=..., use_flexitext_titles=...):
...
def matplotlib_configuration_update(is_interactive: bool, backend: Optional[str] = ...):
...
@contextlib.contextmanager
def matplotlib_configuration(is_interactive: bool, backend: Optional[str] = ...):
...
def resize_window_to_inches(window, width_inches, height_inches, dpi=...):
